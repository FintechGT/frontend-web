...
import { ArrowLeft, FileText, Download, CheckCircle2, Clock, Shield, Loader2, AlertCircle, PenTool } from "lucide-react";
...
export default function ContratoDetallePage() {
  ...
  const handleGuardarFirma = async (dataUrl: string) => {
    ...
    try {
      setFirmando(true);
      const ip = "0.0.0.0"; // en prod, leer IP real
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <div className="mt-1 font-mono text-xs">{(contrato.hash_doc || "").slice(0, 16)}...</div>
          </div>

          <div>
            <div className="text-sm text-neutral-400">Fecha de Creación</div>
            <div className="mt-1">
              {contrato.created_at
                ? new Date(contrato.created_at).toLocaleString("es", {
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                    hour: "2-digit",
          ...
          <div>
            ...
            <div className="mt-1">
              {contrato.updated_at
                ? new Date(contrato.updated_at).toLocaleString("es", {
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                    hour: "2-digit",
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
        ...
        <div className="aspect-[8.5/11] overflow-hidden rounded-lg border border-white/10 bg-white">
          <iframe src={contrato.url_pdf} className="h-full w-full" title="Vista previa del contrato" />
        </div>
      </div>
